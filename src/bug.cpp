...
void Bug::BoundaryFollow(Environment &environment) {
  double distanceTempGoal =
      getEuclideanDistance(this->location, this->boundaryGoal);
  ...
  /**
   * if we are about to reach the end of the boundary, we will check for
   * visited, leave for goal, and change the boundary (in that order)
   */
  else
#endif
  {
    ...
    if (!canLeave) // change direction to other side of the obstacle
    {
      ...
      environment.getNextBoundaryLine(this->location, this->onBoundary,
                                      this->boundaryGoal);
    }
...
bool Bug::canLeaveBoundary(Environment &environment) {
  this->heading =
      getDirectionAngleRadian(environment.getGoal(), this->location);
  Point newLocation;
  ...
  // the way is clear till goal or we can atleast move one step in goal's
  // direction if(!isRouteBlocked || !environment.isObstructed(newLocation))
...
bool Bug::isCrossingPaths(Point goal) {
  ...
  for (int i = 1; i < this->path.size(); ++i) {
    ...
    if (isIntersecting) {
      //			if(intersection.x == line.start.x &&
      //intersection.y == line.start.y ) 				continue;
      return true;
    }
  }
  return false;
...
void Bug::TowardsGoal(Environment &environment, vector<Bug> &bugList) {
  ...
  bool isRouteBlocked = environment.getObstacleIntersection(
      line, intersection, distIntersection, intersectingBoundaryID,
      this->location);

  //	if (this->location.x == intersection.x && this->location.y ==
  //intersection.y) {
  ...
  if (isRouteBlocked) // if there is a line in the way we can move to that line
                      // without any issue
  {
  ...
  //getDirectionAngleRadian(environment.getGoal(),this->location);
  //
  //	//if heading is much different than previous heading then add point to
  //path 	if(abs(newHeading - this->heading) > HEADING_ERROR)
  //	{
  //		this->path.push_back(this->location);
  //	}
  //
...
void KillBugs(vector<Bug> &bugList, Bug &bug) {
  for (int i = bugList.size() - 1; i >= 0; --i) {
    ...
    if (bugList[i].getState() == Bug::FINISHED ||
        bugList[i].getState() == Bug::STUCK ||
        bugList[i].getState() == Bug::RE_VISITING) {
...
Bug::State Bug::getState() { return this->state; }

vector<Point> Bug::getpath() { return this->path; }
double Bug::getCost() { return this->cost; }
Point Bug::getLocation() { return this->location; }
void Bug::setCost(double cost) { this->cost = cost; }

void Bug::setPath(vector<Point> path) { this->path = path; }
